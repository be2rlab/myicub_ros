...
class externalMemory:
    def __init__(self, size=500, name='ext_mem.txt', folder='external_memory'):
        self.fname = name
        self.iterations = 0
        self.size = size
        self.rows = [''] * self.size
        ...
        self.file = str(f'{self.cwd}/{self.folder}/{self.fname}')


        if not os.path.isdir(self.folder):
            os.mkdir(self.folder)

        if not os.path.isdir(f'{self.folder}/images'):
            os.mkdir(f'{self.folder}/images')

        if not os.path.isdir(f'{self.folder}/labels'):
            os.mkdir(f'{self.folder}/labels')
    ...
    def update_memory(self, datafile, update_iters=1):
        ...
        for f in data_to_inject:
            shutil.copyfile(f[:-1], f"{self.cwd}/{self.folder}/images/{f[:-1].split('/')[-1]}")
            txt_file = f[:-1].replace('images', 'labels').replace('png', 'txt')
            shutil.copyfile(txt_file, f"{self.cwd}/{self.folder}/labels/{txt_file.split('/')[-1]}")
